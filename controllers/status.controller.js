...
const create = async (req, res) => {
  try {
    let { error, value } = statusValidation(req.body);
...
const getAll = async (req, res) => {
  ...
  try {
    const data = await pool.query(`SELECT * FROM status LIMIT $1 OFFSET $2`, [
      limit,
      (offset - 1) * limit,
    ]);
...
const getOne = async (req, res) => {
  const { id } = req.params;
  try {
    const data = await pool.query(`SELECT * FROM status WHERE id=$1`, [id]);
